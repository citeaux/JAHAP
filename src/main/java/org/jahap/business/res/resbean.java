...
public class resbean extends DatabaseOperations implements res_i {
    ...
    public resbean(){
        ...
        try {
            ...
            query_AllDbRecords = dbhook.getEntity().createQuery("select t from Res t");
    ...
    public List<Res>SearchForReservations(String searchstring){
     ...
     private void saveNewRecord(){
        if ( newEmptyRecordCreated=true){
            ...
            allrecordlist.clear();
            query_AllDbRecords = dbhook.getEntity().createQuery("select t from Res t"); // Refresh list
            allrecordlist= query_AllDbRecords.getResultList();
    ...
    public Res getDataRecord(long id){
       int inl=0;
        ...
        do 
           ...
           inl++;
           if(allrecordlist.size()<inl){
                     throw new IllegalArgumentException("record does not exists");
           }
       }while(allrecordlist.size()>inl || allrecordlist.get(currentRecordNumber).getId()!=id );
